 ...
 */
public class CropAvatorOverlayView extends View implements CropPhotoViewAttacher.IGetImageBounds {

	private static final String TAG = "CropAvatorOverlayView";
	...
	public CropAvatorOverlayView(Context context) {
		super(context);
		this.mContext = context;
		init();
	}

	public CropAvatorOverlayView(Context context, AttributeSet attrs) {
		super(context, attrs);
		this.mContext = context;
		TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.CropOverlayView, 0, 0);
		try {
			mWidth = ta.getDimensionPixelSize(R.styleable.CropOverlayView_joy_overlay_width, getContext().getResources().getDisplayMetrics().widthPixels);
